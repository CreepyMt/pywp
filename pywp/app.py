...
class Application:
    ...
    def __init__(self, pottype:type=None, potential:potential.Potential=None, partitioner=None, partition_titles=None, boundary=None, wp_generator=None, analyzer=None, basistype=None):
        """ pottype: class name of the potential.
            ...
            basistype: list of basis types. 'cartesian' or 'legendre', number of legendre basis must less than 1. 
        """
    ...
    def parse_args(self, args=sys.argv[1:]):
        ...
        parser.add_argument('--mass', help='mass', type=float)
        parser.add_argument('--mass_y', help='mass', type=float)
        parser.add_argument('--mass_list', help='mass multiple dimensional', type=parsefloatlist)
        parser.add_argument('--init_x', help='init x', type=float)
        parser.add_argument('--init_y', help='init y', type=float)
        parser.add_argument('--init_r', help='init position', type=parsefloatlist)
        parser.add_argument('--sigma_x', help='sigma x', type=float)
        ...
        parser.add_argument('--init_px', help='init px', type=float)
        ...
        parser.add_argument('--init_j', help='init angular momentum', type=int)
    ...
    def set_args(self, box:list, grid:list, mass:float, init_r:list, init_p:list, sigma:list, init_j, init_c, Nstep:int, dt:float, potential_params:list=[],
        ...
        Args:
            ...
            - mass_list: `list[float]`. Mass of the nuclei. 
            - init_j: `int`. Initial angular momentum.  
        """
        self.args = argparse.Namespace(box=box, grid=grid, mass=mass, init_r=init_r, init_p=init_p, sigma=sigma, init_c=init_c, Nstep=Nstep,
            ...
            xwall_right=xwall_right, rtol=rtol, output=output, traj=traj, gpu=gpu, init_j=init_j)
    ...
    def run(self):
        ...
        def get_multidim_arg(args, argname, argname1d, argname2d, dim):
            if getattr(args, argname) is not None:
                ...
                if dim == 1:
                    ...
                    if arg2d: ## not working if arg2d is 0 ... 
        ...
        mass = get_multidim_arg(args, 'mass_list', 'mass', 'mass_y', pot.get_kdim())
        init_j = getattr(args, 'init_j', None)
        ...
        if self.wp_generator is None:
            if self.basistype is None:
                self.wp_generator = lambda R: np.exp(sum([1j*p*R_ - (R_-r)**2/s**2 for (r, p, R_, s) in zip(init_r, init_p, R, sigma)]))
            else:
                def wp_generator_jacobi(R,U):
                    psi = np.zeros_like(R[0],dtype=complex)
                    psi[...,init_j] = np.exp(sum([1j*p*R_[...,0] - (R_[...,0]-r)**2/s**2 for (r, p, R_, s) in zip(init_r, init_p, R, sigma)])) 
                    psi = np.einsum('jk,...k->...j',U.T.conj(),psi)
                    return psi 
                self.wp_generator = wp_generator_jacobi
        preproc_args = preprocess(pot, grid, box, self.wp_generator, init_c, mass, args.dt)
